...
if options.usercode:
  ...
  if options.js_varname:
    ...
    output = pg_logger.exec_script_str_local(fin.read(), options.raw_input_lst_json, options.cumulative, options.heapPrimitives, js_var_finalizer)
  else:
    output = pg_logger.exec_script_str_local(fin.read(), options.raw_input_lst_json, options.cumulative, options.heapPrimitives, json_finalizer)
  outfile = open(args[1], "w") if len(args) > 1 else sys.stdout
